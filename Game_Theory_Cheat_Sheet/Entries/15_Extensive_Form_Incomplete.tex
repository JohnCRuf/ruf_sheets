...
the game has perfect recall. Then: $\forall \sigma_i \in \Sigma_{i}$, $\exists \alpha \in \Delta\left(\Sigma_{i}^{p}\right)$ s.t.\ $\alpha$ and $\sigma_{i}$ are equivalent.

\subsection{Kuhn's Theorem}
Suppose the game has perfect recall, then for any $\alpha_i \in \Delta(\Sigma_i^P)$, $\exists \sigma_i \in \Sigma_i $ s.t. $\alpha_i$ and $\sigma_i$ are equivalent. This goes in the opposite direction of the previous proposition.
...
Player i's expected utility from $\sigma$ starting at h
$$
U_{i}(\sigma, h)=\sum_{h^{\prime} \in Z} u_{i}\left(h^{\prime}\right) \mu\left(h^{\prime} \mid h, \sigma\right)
$$
\subsection{Nash Equilibrium}
$\sigma$ is a Nash equilibrium if $\forall$ i, $\sigma_i' \in \Sigma_i$
$$
u_{i}\left(\sigma, \emptyset \right) \geq u_{i}\left(\sigma_{i}', \sigma_{-i}, \emptyset\right) \text {. }
$$
\subsection{Interim Beliefs and Assessments}
An interim belief for player $i$ is a mapping
...
An assessment is sequentially rational if $\forall i, \ p_i \in P_i, \ \sigma'_i \in \Sigma_i$:
...
\subsection{When to Use Sequential Equilibrium}
\begin{itemize}[nosep]
    \item Use when the game is \textbf{sequential} with \textbf{incomplete information} (non-trivial information sets).
    \item If all information sets are singletons (perfect info), SPE suffices---sequential equilibrium reduces to SPE.
    \item \textbf{Practical approach:} (1) Guess a strategy profile. (2) Compute beliefs via Bayes' rule on the path. (3) For off-path info sets, assign beliefs that support the strategy. (4) Check sequential rationality at every info set.
    \item \textbf{Consistency check:} beliefs must be the limit of Bayes' rule under fully mixed perturbations. Off-path beliefs are pinned down by the trembling sequence, not freely chosen.
\end{itemize}
