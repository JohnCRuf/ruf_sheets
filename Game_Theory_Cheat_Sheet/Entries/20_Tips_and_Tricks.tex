...
From the notes, we can also note that given a behavioral strategy $\sigma$, we can also derive the payoff difference between $\sigma$ and an deviation $\sigma'$ (from the definition of $\sigma_i'$ above) as:

\begin{align*}
    U_i(\sigma) - U_i(\sigma_i', \sigma_{-i})&= \sum_{\omega \in \Omega} \sum(a \in A) (\sigma(a|w)- (\sigma_i' \sigma_{-i})(a|\omega)) \pi(\omega) u_i(a) \\
...
%%% From Decision \& Strategy II %%%
\subsection{Sequential Equilibria}

\begin{itemize}
    \item (From PSet 5) If an assessment \((\beta,\, \sigma)\) is consistent, it is sequentially rational if and only if it has no one-shot deviations. Here, we understand an OSD to be a strategy \(\sigma'_i\) which differs from \(\sigma_i\) at exactly one information set.
\end{itemize}
