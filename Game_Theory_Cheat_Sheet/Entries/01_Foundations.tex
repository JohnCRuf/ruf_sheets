\section{Lecture 1: Basics of Game Theory}
...
A relation, R,   have the following properties: \\
\textbf{Completeness}
\begin{align*}
...
\begin{itemize}
    ...
    \item If $\succeq$ is complete and anti-symmetric then $\succ$ is total (pset1)
\end{itemize}
Propositions about utility and preferences

\textbf{Proposition: If $\succeq$ is represented by U then $\succeq$ is rational.} \\
Note: This does not go in the opposite way. proved in class w/ counterexample:

\subsection{Rational implies U counterexample}
Let $X=[0,1]$ and we have lexicographic preferences, ie. $(x,y) \succeq (x',y')$ if $x \geq x'$ or $x=x'$ and $y \geq y')$. This is derived in class by contradiction in that a utility function existing implies that the cardinality of [0,1] is weakly less than the rationals. 
