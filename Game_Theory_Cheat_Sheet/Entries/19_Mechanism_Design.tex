...
For the design of an auction to be revenue-optimal, we must have that \(U(\underline{v}) = 0\). \textbf{Assumption (Myerson regularity):} the virtual value \(\varphi_i\) is non-decreasing in \(v_i\). Under symmetry and regularity, expected revenue is maximized by SPA/FPA with reserve price \(r^* = \min\{x | \varphi(x) \geq 0\}\).
