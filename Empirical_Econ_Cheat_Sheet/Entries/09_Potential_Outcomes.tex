...
\end{align*}
\textbf{Fundamental problem:} never observe both $y_i(1)$ and $y_i(0)$.
...
\end{align*}
Naive comparison $=$ ATT only if selection bias $=0$.
...
\end{align*}
Selection bias vanishes, and $\beta_1 = \E(Y|D=1)-\E(Y|D=0) = \text{ATE}$.
...
\textbf{Misspecification trap:} If you omit $D_i x_i$ and run $y=b_0+b_1 x+b_2 D+e$, then $b_2$ converges to a \emph{variance-weighted} average of conditional ATEs:
\begin{align*}
...
\end{align*}
Useful when the propensity score $p(x)$ is easier to model than $\E(Y|D,X)$.
