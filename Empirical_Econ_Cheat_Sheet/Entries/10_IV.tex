...
\item \textbf{Exclusion:} $Y_i(d,z)=Y_i(d)$ $\forall d,z$ (instrument affects $Y$ only through $D$).
\item \textbf{First stage:} $P(D\!=\!1|Z\!=\!1)\neq P(D\!=\!1|Z\!=\!0)$.
\item \textbf{Monotonicity:} $D_i(1)\geq D_i(0)$ $\forall i$ (no defiers).
\end{itemize}
...
\end{align*}
\textbf{Derivation:} Numerator $= \E(Y(1)\!-\!Y(0))$ by independence/exclusion. Always-takers and never-takers contribute zero ($D$ unchanged by $Z$); defiers ruled out by monotonicity. So numerator $= P(\text{c})\cdot\E(Y(1)\!-\!Y(0)|\text{c})$. Denominator $= P(\text{c})$.
...
(A4) $\E|Y_d|<\infty$;
(A5) $0\!<\!P(D\!=\!1|X)\!<\!1$.
Vytlacil (2002): (A1)--(A5) $\iff$ LATE assumptions. The latent variable model generates LATE, and the LATE assumptions generate the latent variable model. Breaks down for multivalued treatments.
