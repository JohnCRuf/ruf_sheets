...
\begin{align*}
    y_i = x_i'\beta + u_i, \quad \E(x_i u_i)=0 \;\text{(exogeneity)}.
\end{align*}
Here $x_i\in\R^{k+1}$ with $x_{i0}=1$. We say the model is ``linear'' because it is linear in the parameters $\beta_j$. The error $u_i$ captures all unobserved determinants of $y_i$. When $\E(xu)\neq 0$, we say $x$ is \textbf{endogenous}.
...
\end{align*}
Equivalently, $\hat\beta_n$ solves $\min_{b}\|Y - Xb\|^2$. The FOC gives $X'\hat{U}=0$.
...
\end{align*}
That is, $\hat\beta_2$ is obtained by regressing the residuals of $Y$ on $X_1$ onto the residuals of $X_2$ on $X_1$.
