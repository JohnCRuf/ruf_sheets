...
Exception: $X_n \dto c$ (constant) $\implies X_n \pto c$.
...
\end{align*}
\textbf{Does NOT hold for $\xrightarrow{r}$}. Counterexample: $X_n=n$ w.p.\ $1/n^2$, $0$ otherwise. $g(x)=x^2$: $\E|X_n|\to 0$ but $\E|X_n^2|=1$.

\textbf{Important:} Need $P(X\in S)=1$. E.g.\ $g(x,y)=x/y$ continuous on $S=\R^2\setminus\{(x,0)\}$; need $c\neq 0$ for $X_n/Y_n\dto X/c$.
...
\end{align*}
\textbf{Critical:} $Y_n$ must converge to a \emph{constant}. If $Y_n\dto Y$ (non-degenerate), Slutsky does not apply.

\subsection{Weak Law of Large Numbers}
If $\{X_i\}_{i\geq 1}$ iid with $\E(X_i)=\mu$, $\Var(X_i)=\sigma^2<\infty$, then $\bar{X}_n \pto \mu$.

\textbf{Proof (Chebyshev):} $P(|\bar{X}_n-\mu|>\epsilon)\leq \frac{\Var(\bar{X}_n)}{\epsilon^2}=\frac{\sigma^2}{n\epsilon^2}\to 0$.
