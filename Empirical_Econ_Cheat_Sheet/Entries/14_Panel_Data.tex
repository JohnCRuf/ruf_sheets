...
\textbf{Consistency requires:} $\E(\Delta X_{it}'\Delta u_{it})=0$, that is, the changes in regressors are uncorrelated with the changes in errors. A sufficient (but stronger than necessary) condition is $\E(u_{it}|X_{it},X_{it-1})=0$; FD only requires the moment condition on the differences, not full contemporaneous exogeneity.
...
Standard errors must account for within-individual serial correlation in $u_{it}$. \textbf{Cluster-robust variance:} allows arbitrary within-cluster correlation:
